...
app.listen(PORT, async () => {
  ...
  // Phase E — Anti-Fraud, Wallet Pro, Payouts, Notifications
  try {
    const { migratePhaseE } = require('./db/migrate-phase-e');
    await migratePhaseE();
  } catch (err) {
  ...
  try {
    const { migrateI18n } = require('./db/migrate-i18n');
    await migrateI18n();
  } catch (err) {
  ...
  try {
    const { migrateAiLeadersV2 } = require('./db/migrate-ai-leaders-v2');
    await migrateAiLeadersV2();
  } catch (err) {
