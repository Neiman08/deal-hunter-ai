...
// POST /api/stores/seed-illinois  (temporary debug — remove after seed confirmed)
router.post('/seed-illinois', async (req, res) => {
  if (req.headers['x-seed-token'] !== 'dh-seed-2026') {
  ...
  for (const s of IL_STORES) {
    try {
      const exists = await query('SELECT id FROM store_locations WHERE store_id=$1 AND address=$2 AND city=$3', [s.store_id, s.address, s.city]);
      if (exists.rows.length > 0) { results.push({ status: 'skip', addr: `${s.address}, ${s.city}` }); continue; }
      await query(
        `INSERT INTO store_locations (store_id, store_number, address, city, state, zip_code, latitude, longitude, is_active)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,true)`,
        [s.store_id, s.store_number, s.address, s.city, s.state, s.zip_code, s.latitude, s.longitude]
      );
      results.push({ status: 'inserted', addr: `${s.address}, ${s.city}` });
    } catch (err) {
      results.push({ status: 'error', addr: `${s.address}, ${s.city}`, error: err.message, code: err.code });
    }
  }
  const inserted = results.filter(r => r.status === 'inserted').length;
  const errors = results.filter(r => r.status === 'error');
  res.json({ inserted, skipped: results.filter(r => r.status === 'skip').length, errors, results });
});
