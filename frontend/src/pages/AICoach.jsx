...
const QUICK_PROMPTS = [
  { label: 'How do I level up?',          msg: 'How do I level up faster?' },
  { label: 'How can I earn more points?', msg: 'How can I earn more points?' },
  { label: "What should I do today?",     msg: 'What should I do today?' },
  { label: 'How can I grow my team?',     msg: 'How can I grow my team?' },
];
...
function SuggestionCard({ s, onAsk }) {
...
export default function AICoach() {
  ...
  useEffect(() => {
    api.get('/business/coach/summary')
      .then(res => {
        const { suggestions: suggs, ...rest } = res.data;
        setSummary(rest);
        setSuggestions(suggs || []);
        setMessages([{
          role: 'assistant',
          content: `Hey ${''}, I'm your Deal Hunter Coach! 👋\n\nYou're currently at **${rest.xp} XP** as a **${rest.level}**.${rest.next_level ? ` You're **${rest.xp_to_next} XP** away from **${rest.next_level}**.` : ' You\'re at the top level!'}\n\nI've analyzed your account and have ${suggs?.length || 0} personalized suggestions for you. What would you like help with today?`,
        }]);
      })
      .catch(err => {
        if (err.response?.status === 401) navigate('/login');
        else setError('Failed to load Coach.');
      })
  ...
  async function send(text) {
    ...
    try {
      const res = await api.post('/business/coach/ask', { message: msg });
    ...
    <div className="flex flex-col h-full max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 p-4 border-b border-dark-700 flex-shrink-0">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-500 text-xs">Smart guidance mode · powered by your data</p>
        </div>
        {summary && (
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 space-y-5">
          ...
          {summary && (
            <div className="rounded-2xl p-4 border border-neon-green/20 bg-neon-green/5">
              <div className="flex items-center gap-3 mb-3">
                <div className="flex-1">
                  <p className="text-neon-green text-xs font-bold uppercase tracking-wider">Your Hunter Status</p>
              ...
              {summary.next_level && summary.xp_to_next > 0 && (
                <div className="mt-3">
                  <div className="flex justify-between text-xs text-gray-500 mb-1">
                    <span>Progress to {summary.next_level}</span>
                    <span>{summary.xp_to_next} XP remaining</span>
                  </div>
          ...
          {suggestions.length > 0 && (
            <div className="space-y-2">
              <h2 className="text-white font-bold text-sm flex items-center gap-2">
                <Zap size={13} className="text-yellow-400" /> Personalized Suggestions
              </h2>
              {suggestions.map((s, i) => (
                <SuggestionCard key={i} s={s} onAsk={msg => send(msg)} />
              ))}
            </div>
          )}
          ...
          <div className="space-y-3">
            <h2 className="text-white font-bold text-sm flex items-center gap-2">
              <Bot size={13} className="text-neon-green" /> Chat with Coach
            </h2>
      ...
      <div className="flex-shrink-0 p-4 border-t border-dark-700 bg-dark-800">
        ...
        <p className="text-center text-gray-600 text-[10px] mt-2">
          Coach IA is running in smart guidance mode · powered by your real data
        </p>
      </div>
    </div>
  );
