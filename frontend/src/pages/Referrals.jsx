    ...
    <div className="p-4 lg:p-6 space-y-6 max-w-2xl mx-auto">
      ...
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-3">
          <Gift size={22} className="text-neon-green" /> {t('referrals.title')}
        </h1>
        <p className="text-gray-300 text-sm mt-0.5">{t('referrals.subtitle')}</p>
      </div>

      {/* How it works */}
      <div className="card border-neon-green/20 bg-neon-green/5">
        <p className="text-neon-green text-sm font-semibold mb-3">{t('referrals.howItWorks')}</p>
        <div className="grid grid-cols-3 gap-3 text-center">
          {[
            { step: '1', icon: '🔗', label: t('referrals.steps.share') },
            { step: '2', icon: '👤', label: t('referrals.steps.signup') },
            { step: '3', icon: '💰', label: t('referrals.steps.earn') },
          ].map(s => (
            <div key={s.step}>
      ...
      {/* Your code */}
      <div className="card">
        <p className="text-gray-400 text-xs uppercase tracking-wider mb-3">{t('referrals.yourCode')}</p>
        ...
        <p className="text-gray-400 text-xs mb-2">{t('referrals.referralLink')}</p>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { icon: <Users size={16} />, label: t('referrals.stats.total'), value: data?.stats?.total_signups || 0, color: 'text-neon-blue' },
          { icon: <Star size={16} />, label: t('referrals.stats.converted'), value: data?.stats?.conversions || 0, color: 'text-neon-green' },
          { icon: <DollarSign size={16} />, label: t('referrals.stats.earned'), value: data?.stats?.months_earned || 0, color: 'text-yellow-400' },
        ].map(s => (
      ...
      <div className="card">
        ...
        <div className="space-y-2">
          {(() => {
            const tierMeta = [
              { icon: '🎁', color: 'text-gray-300',   bg: 'bg-dark-800',       border: 'border-dark-700' },
              { icon: '⭐', color: 'text-neon-green', bg: 'bg-neon-green/5',   border: 'border-neon-green/20' },
              { icon: '💵', color: 'text-neon-blue',  bg: 'bg-neon-blue/5',    border: 'border-neon-blue/20' },
              { icon: '💰', color: 'text-yellow-400', bg: 'bg-yellow-400/5',   border: 'border-yellow-400/20' },
              { icon: '👑', color: 'text-purple-400', bg: 'bg-purple-400/5',   border: 'border-purple-400/20' },
            ];
            const tiers = data?.tiers || [
              { threshold: 1,  label: '7 days Pro free',    reached: false, remaining: 1 },
              { threshold: 3,  label: '1 month Pro free',   reached: false, remaining: 3 },
              { threshold: 5,  label: '$10 account credit', reached: false, remaining: 5 },
              { threshold: 10, label: '$25 account credit', reached: false, remaining: 10 },
              { threshold: 25, label: 'Lifetime Pro',       reached: false, remaining: 25 },
            ];
            return tiers.map((tier, i) => {
              const m = tierMeta[i] || tierMeta[tierMeta.length - 1];
              return (
                <div key={tier.threshold} className={`flex items-center gap-3 p-3 rounded-xl border ${m.bg} ${m.border} ${!tier.reached ? 'opacity-60' : ''}`}>
                  <span className="text-xl w-7 text-center">{m.icon}</span>
                  <div className="flex-1">
                    <p className={`text-sm font-semibold ${m.color}`}>{tier.label}</p>
                  ...
                  {tier.reached ? (
                    ...
                    <span className="text-gray-500 text-xs">{tier.remaining} more</span>
                  )}
                </div>
              );
            });
        ...
        <p className="text-gray-500 text-xs mt-3">Rewards applied automatically within 24 hours of conversion.</p>
      </div>

      {/* Recent referrals */}
      {data?.recent?.length > 0 && (
        <div className="card">
          <h3 className="text-white font-semibold mb-3">{t('referrals.recent')}</h3>
          <div className="space-y-2">
            {data.recent.map((r, i) => (
